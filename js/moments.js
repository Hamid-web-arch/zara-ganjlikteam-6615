document.addEventListener("DOMContentLoaded", () => {
    ...
    if (exploreMomentsBtn) {
        exploreMomentsBtn.addEventListener("click", (e) => {
            ...
            // Əgər SPA artıq yüklənibsə, sadəcə görünüşü aç və yaddaşdakı videoları bərpa et
            if (spaRoot.innerHTML.trim() !== "") {
                openMomentsView();
                loadApprovedVideos(); // Təsdiqlənmiş videoları yoxla və yüklə
                return;
            }
            ...
            fetch("moments.htm")
                ...
                .then(htmlContent => {
                    ...
                    // Adminin təsdiqlədiyi real videoları default videoların üstünə yükləyirik
                    loadApprovedVideos();
                    
                    // VİDEO AVTOMATİK OYNATMANI BURADA AKTVİLƏŞDİRİRİK!
                    setTimeout(initVideoAutoplay, 150); 
                })
    ...
    function openMomentsView() {
        ...
        if (platform) {
            ...
            const video = platform.querySelector("video");
    ...
    function loadApprovedVideos() {
        ...
        // Əvvəlcə əgər köhnədən qalan dinamik yüklənmiş kartlar varsa, dublikat olmasın deyə təmizləyirik
        // (Ancaq sinifində 'dynamic-card' olanları silirik ki, htm-dəki default-lar itməsin)
        document.querySelectorAll(".zara-card.dynamic-card").forEach(el => el.remove());

        // Admin tərəfindən təsdiqlənmiş real videoları çəkirik
        const approvedVideos = JSON.parse(localStorage.getItem("zara_approved_moments")) || [];
        
        // Təsdiqlənən videoları tərsinə dövr edirik ki, ən son təsdiqlənən ən üstdə görünsün
...
`;
            // Default videoların EN ÜSTÜNƏ əlavə edirik
            container.insertAdjacentHTML("afterbegin", cardHTML);
        });

        // Autoplay-i yenidən tetikləyirik ki, yeni gələn kartları tanısın
        initVideoAutoplay();
    }
    ...
    document.addEventListener("click", (e) => {
        ...
        // PLATFORMADAN ÇIXIŞ (Geri düyməsi)
        ...
        // ========================================================
        // 9. CLOUDINARY VİDEO YÜKLƏMƏ (BİRBAŞA ADMİN PANELƏ GÖNDƏRİR)
        // ========================================================
        if (e.target.id === "uploadForm" || e.target.closest("#uploadForm")) {
            const form = document.getElementById("uploadForm");
            ...
            form.onsubmit = function(submitEvent) {
                ...
                .then(data => {
                    ...
                    // 🚨 BURADA VİDEONU PLEYERƏ ATXALMIRIQ! ADMİNİN NÖVBƏSİNƏ (PENDING) ƏLAVƏ EDİRİK!
...
function initVideoAutoplay() {
    ...
    // 🚨 YENİ STRATEGİYA: İstifadəçinin Moments platformasında ilk dəfə harasa kliklədiyini izləmək üçün flag
    // Bu, ilk videonun səsli və kliklə açılmasını təmin edəcək
    ...
    const videoObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            // Əgər video ekrana daxil olubsa (Skrol edilibsə və ya ilk videodursa)
            if (entry.isIntersecting) {
                ...
                // Əgər istifadəçi hələ pleyerə heç toxunmayıbsa (Səhifə ilk dəfə açılıbsa)
                if (!window.hasInteracted) {
                    video.pause(); // Videonu dondur
                    video.muted = false; // Səsi açıq saxla ki, klikləyəndə birbaşa səsli gəlsin
                    if(playIcon) playIcon.className = "fa-solid fa-play";
                    
                    // Ortadakı böyük Play düyməsini görünən et (İstifadəçi bilsin ki, klikləməlidir)
                    if(overlayIcon) {
                        overlayIcon.classList.remove("opacity-0", "scale-75");
                        overlayIcon.classList.add("opacity-100", "scale-100");
                    }
                } else {
                    // Əgər istifadəçi artıq bir dəfə klikləyibsə, digər videolara keçəndə avtomatik səsli oynasın
                    video.muted = false;
                    video.play().catch(() => {
                        // Əgər brauzer hələ də təhlükəsizlik xətası versə, səssizə alıb oynat
                        video.muted = true;
                        video.play().catch(() => {});
                    });
                ...
                // --- IDARƏƏTMƏ DÜYMƏLƏRİNİN KLİK MEXANİZMLƏRİ ---
                if (progressContainer) {
                    progressContainer.onclick = (e) => {
                        e.stopPropagation();
                        window.hasInteracted = true; // İstifadəçi toxundu!
                ...
                // Videonun öz üzərinə klik edəndə açılması
                video.onclick = (e) => {
                    e.stopPropagation();
                    window.hasInteracted = true; // 🚨 Artıq istifadəçi bircə dəfə kliklədi, flag-i true edirik!
                    video.muted = false; // Səsi mütləq aç
                    togglePlayPause(video, playIcon, overlayIcon);
                };

            } else {
                // Video ekrandan çıxanda dayandır və sıfırla
                video.pause();
                video.currentTime = 0;
    ...
    // PLAY / PAUSE MEXANİZMİ
    function togglePlayPause(video, playIcon, overlayIcon) {
        if (video.paused) {
            video.muted = false; // Kliklənib açıldığı üçün səs aktiv olur
            video.play().catch(() => {
                video.muted = true;
                video.play().catch(() => {});
            });
            ...
            // İkon animasiyası (Gəlib-itmə effekti)
            if(overlayIcon) {
